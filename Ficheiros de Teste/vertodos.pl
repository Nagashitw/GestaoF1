pesqresultado:-nl,consult('piloto.pl'),consult('resultados.pl'),mostrar_verifica_resultado_memoria(ListaResultados).
																	%pesquisar_dados_resultado(ListaResultados).



	mostrar_verifica_resultado_memoria(ListaResultados):-findall(
																		resultados(ID_Pista,dataprova(Dia,Mes,Ano),ListaResultados),
																		resultados(ID_Pista,dataprova(Dia,Mes,Ano),ListaResultados),	
																		[]),
																!,fail.

mostrar_verifica_resultado_memoria(ListaResultados):-findall(resultados(ID_Pista,dataprova(Dia,Mes,Ano),ListaResultados),resultados(ID_Pista,dataprova(Dia,Mes,Ano),ListaResultados),ListaResultados),pesquisar_dados_resultado(ListaResultados).
